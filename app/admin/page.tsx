...
import { getCrefSearchUrl } from "@/lib/cref";
...
type AdminUser = {
  ...
  crefRegion: string | null;
  crefVerified: boolean | null;
};
...
const ACTION_LABELS: Record<string, string> = {
  ...
  verifyCref: "verificou o CREF de",
  unverifyCref: "removeu a verificação de CREF de",
};
            ...
            {users.map((u) => (
              ...
              >
                <div>
                  ...
                  {u.role === "personal" && (
                    <p className="text-xs mt-1">
                      <span
                        className={
                          u.crefVerified ? "text-teal" : "text-chalk/50"
                        }
                      >
                        {u.crefVerified ? "✓ CREF verificado" : "CREF pendente"}
                        {u.crefNumber && ` — ${u.crefNumber}`}
                        {u.crefRegion && ` (${u.crefRegion})`}
                      </span>
                      {u.crefNumber && (
                        <a
                          href={getCrefSearchUrl(u.crefRegion)}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-teal underline ml-2"
                        >
                          conferir
                        </a>
                      )}
                ...
                <div className="flex flex-wrap gap-2">
                  {u.role === "personal" && u.crefNumber && (
                    <button
                      onClick={() =>
                        handleAction(
                          u.id,
                          u.crefVerified ? "unverifyCref" : "verifyCref"
                        )
                      }
                      disabled={actionId === u.id}
                      className="text-xs border border-teal/40 text-teal rounded-full px-3 py-1 hover:bg-teal/10 transition disabled:opacity-50"
                    >
                      {u.crefVerified ? "Remover verificação" : "Verificar CREF"}
                    </button>
                  )}
                  {!u.isAdmin && (
                    <button
